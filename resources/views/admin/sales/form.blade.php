...
<option value="{{ $stock->product_id }}, {{ $stock->id }}">{{ $stock->product->manufacturer->name ?? 'No Product' }} {{ $stock->product->name ?? 'No model' }} ({{ $stock->product->category->name }}) | {{ __('Current Quantity') }} - <strong>{{ $stock->quantity_in_hand }}</strong></option>
@endforeach
    @endif
    </select>
